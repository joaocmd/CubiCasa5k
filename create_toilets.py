...
def main(data_folder, split, output_folder):
    ...
    for val in tqdm(data_iter):
        ...
        toilets = [e for e in svg.getElementsByTagName("g") if e.getAttribute("class") == "FixedFurniture Toilet"]

        for toilet_idx, d in enumerate(toilets):
            cropped = crop_toilet(image, d)

            cv2.imwrite(f'{output_folder}/{folder[1:-1].replace("/","_")}_{toilet_idx}.png', (cropped*255).astype(np.uint8))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out-folder', nargs='?', type=str, default='toilets/')
