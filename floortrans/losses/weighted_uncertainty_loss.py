...
class WeightedUncertaintyLoss(Module):
    ...
    def forward(self, input, target):
        ...
        self.loss_rooms_var = self.weighted_ce(rooms_pred*torch.exp(-self.log_vars[0]), rooms_target)
        self.loss_icons_var = self.weighted_ce(icons_pred*torch.exp(-self.log_vars[1]), icons_target)

        self.loss_rooms = self.weighted_ce(rooms_pred, rooms_target)
        self.loss_icons = self.weighted_ce(icons_pred, icons_target)
    ...
    def weighted_ce(self, pred, target):
        num_classes = pred.shape[1]
        N = pred.shape[2] * pred.shape[3]
        weights = torch.FloatTensor([N - (target == i).sum() for i in range(num_classes)]).to(self.device)
        return cross_entropy(input=pred, target=target, weight=weights, reduction='sum')/weights.sum()
